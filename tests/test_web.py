...
def test_static_main_js_translation_spacing():
    ...
    assert response.status_code == 200
    assert 'SPA</span> <span class="text-gray-500' in response.text
    assert 'ENG</span> <span class="text-gray-500' in response.text
    assert "${langLabel}</span> <span class=\"text-gray-500" in response.text
